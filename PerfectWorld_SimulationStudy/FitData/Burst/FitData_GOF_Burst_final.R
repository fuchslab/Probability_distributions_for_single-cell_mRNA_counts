...
save(minBIC,p_x2, file=paste0("GOFData/Burst/p_gof_burst_",i,".rda"))
}

#Burst Fit

load(file="GenData/Data_burst.Rda")
i <- as.numeric(commandArgs(TRUE))

filename <- paste0("FitData/Burst/burst_",i,".rda")
if(!file.exists(filename)){
# do complete fit
set.seed(i*5+5)
#name of listentry
listname <- paste(paste("Sample ",bquote(.(i))))
main_data <- Data_burst[[listname]]
