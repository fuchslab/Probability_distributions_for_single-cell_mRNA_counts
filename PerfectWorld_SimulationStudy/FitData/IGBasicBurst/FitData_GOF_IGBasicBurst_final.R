...
save(minBIC,p_x2, file=paste0("GOFData/IGBasicBurst/p_gof_IGbasicburst_",i,".rda"))}

#Basic Fit

load(file="GenData/Data_IGbasic_burst.Rda")
i <- as.numeric(commandArgs(TRUE))

filename <- paste0("FitData/IGBasicBurst/IGbasicburst_",i,".rda")
if(!file.exists(filename)){
# do complete fit
set.seed(i*5+5)
#name of listentry
listname <- paste(paste("Sample ",bquote(.(i))))
main_data <- Data_IGbasic_burst[[listname]]
