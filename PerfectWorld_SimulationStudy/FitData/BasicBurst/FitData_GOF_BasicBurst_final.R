...
save(minBIC,p_x2, file=paste0("GOFData/BasicBurst/p_gof_basicburst_",i,".rda"))
}

#Basic Fit

load(file="GenData/Data_basic_burst.Rda")
i <- as.numeric(commandArgs(TRUE))

filename <- paste0("FitData/BasicBurst/basicburst_",i,".rda")
if(!file.exists(filename)){
# do complete fit
set.seed(i*5+5)
#name of listentry
listname <- paste(paste("Sample ",bquote(.(i))))
main_data <- Data_basic_burst[[listname]]
