...
#################################################################################
#NB2
pDEL2 <- function(q, p, mu1, sigma1, nu1, mu2, sigma2, nu2){
    return(p * pDEL(q,mu = mu1, sigma =  sigma1, nu = nu1)+(1-p)*pDEL(q, mu = mu2, sigma = sigma2, nu = nu2))
}
dDEL2 <- function(x, p, mu1, sigma1, nu1, mu2, sigma2, nu2){
    return(p * dDEL(x,mu = mu1, sigma =  sigma1, nu = nu1)+(1-p)*dDEL(x, mu = mu2, sigma = sigma2, nu = nu2))
}
...
for(id in names(which(comparison_all_name=="del2")) ){
    ...
    p1 <- dDEL2(0:max(main_data[id,]),p=Par[[id]][1],mu1=Par[[id]][2], sigma1= Par[[id]][3], nu1= Par[[id]][4], mu2=Par[[id]][5], sigma2= Par[[id]][6], nu2= Par[[id]][7])
    ...
    p_ks[[id]] <- ks.test(main_data[id,],"pDEL2",p=Par[[id]][1],mu1=Par[[id]][2], sigma1= Par[[id]][3], nu1= Par[[id]][4], mu2=Par[[id]][5], sigma2= Par[[id]][6], nu2= Par[[id]][7])


}
save(p_ks,p_x2, file="GOF/p_gof_del2.rda")
