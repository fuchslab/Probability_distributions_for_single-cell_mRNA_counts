main_data <- readRDS("/storage/groups/biostat01/projects/Lisa_ProbDistr/RealWorld/mm10/mm10/mm10.rds")
...
#################################################################################
#ZIPIG2
pZIPIG2 <- function(q, p_1,p_2, mu1, sigma1, mu2, sigma2){
    return(p_1 * (q>=0) + p_2 * pPIG(q,mu = mu1,sigma =  sigma1)+(1-p_1-p_2)*pPIG(q, mu = mu2,sigma = sigma2))
}
dZIPIG2 <- function(x, p_1,p_2, mu1, sigma1, mu2, sigma2){
    return(p_1 * (x==0) + p_2 * dPIG(x,mu = mu1,sigma =  sigma1)+(1-p_1-p_2)*dPIG(x, mu = mu2,sigma = sigma2))
}
...
for(id in names(which(comparison_all_name=="zipig2")) ){
    ...
    p1 <- dZIPIG2(0:max(main_data[id,]),p_1=Par[[id]][1],p_2=Par[[id]][2],mu1=Par[[id]][3], sigma1= Par[[id]][4],mu2=Par[[id]][5], sigma2= Par[[id]][6])
    ...
    p_ks[[id]] <- ks.test(main_data[id,],"pZIPIG2",p_1=Par[[id]][1],p_2=Par[[id]][2],mu1=Par[[id]][3], sigma1= Par[[id]][4],mu2=Par[[id]][5], sigma2= Par[[id]][6])


}
save(p_ks,p_x2, file="GOF/p_gof_zipig2.rda")
