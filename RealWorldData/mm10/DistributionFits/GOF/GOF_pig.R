...
for(id in names(which(comparison_all_name=="pig")) ){
    ...
    p1 <- dPIG(0:max(main_data[id,]),mu=Par[[id]][1],sigma=Par[[id]][2])
...
p_ks[[id]] <- ks.test(main_data[id,],"pPIG",mu=Par[[id]][1],sigma=Par[[id]][2])


}

save(p_ks,p_x2, file="GOF/p_gof_pig.rda")
