...
#################################################################################
#ZIDEL 
pZIDEL <- function(q, p, mu1, sigma1, nu1){
    return(p * (q >= 0) + (1 - p) * pDEL(q, mu = mu1,sigma =  sigma1, nu = nu1))
}
dZIDEL <- function(x, p, mu1, sigma1, nu1){
    return(p * (x == 0) + (1 - p) * dDEL(x, mu = mu1,sigma =  sigma1, nu = nu1))
}
for(id in names(which(comparison_all_name=="zidel")) ){
    ...
    p1 <- dZIDEL(0:max(main_data[id,]), p = Par[[id]][1], mu1 = Par[[id]][2], sigma1 = Par[[id]][3], nu1 = Par[[id]][4])
    ...
    p_ks[[id]] <- ks.test(main_data[id,],"pZIDEL",p=Par[[id]][1],mu1=Par[[id]][2], sigma1= Par[[id]][3], nu1= Par[[id]][4])


}

save(p_ks,p_x2, file="GOF/p_gof_zidel.rda")
