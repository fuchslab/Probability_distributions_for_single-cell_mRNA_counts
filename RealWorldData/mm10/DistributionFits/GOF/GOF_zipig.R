...
for(id in names(which(comparison_all_name=="zipig")) ){
    ...
    p1 <- dZIPIG(0:max(main_data[id,]),nu=Par[[id]][1],mu=Par[[id]][2], sigma= Par[[id]][3])
    ...
    p_ks[[id]] <- ks.test(main_data[id,],"pZIPIG",nu=Par[[id]][1],mu=Par[[id]][2], sigma= Par[[id]][3])


}

save(p_ks,p_x2, file="GOF/p_gof_zipig.rda")
