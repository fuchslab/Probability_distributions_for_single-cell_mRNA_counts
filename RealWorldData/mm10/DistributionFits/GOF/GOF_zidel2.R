...
#################################################################################
#ZIDEL2
pZIDEL2 <- function(q, p_1,p_2, mu1, sigma1, nu1, mu2, sigma2, nu2){
    return(p_1 * (q>=0) + p_2 * pDEL(q,mu = mu1,sigma =  sigma1, nu = nu1)+(1-p_1-p_2)*pDEL(q, mu = mu2,sigma = sigma2, nu = nu2))
}
dZIDEL2 <- function(x, p_1,p_2, mu1, sigma1, nu1, mu2, sigma2, nu2){
    return(p_1 * (x==0) + p_2 * dDEL(x,mu = mu1,sigma =  sigma1, nu = nu1)+(1-p_1-p_2)*dDEL(x, mu = mu2,sigma = sigma2, nu = nu2))
}
...
for(id in names(which(comparison_all_name=="zidel2")) ){
    ...
    p1 <- dZIDEL2(0:max(main_data[id,]),p_1=Par[[id]][1],p_2=Par[[id]][2],mu1=Par[[id]][3], sigma1= Par[[id]][4],nu1=Par[[id]][5],mu2=Par[[id]][6], sigma2= Par[[id]][7],nu2=Par[[id]][8])
    ...
    p_ks[[id]] <- ks.test(main_data[id,],"pZIDEL2",p_1=Par[[id]][1],p_2=Par[[id]][2],mu1=Par[[id]][3], sigma1= Par[[id]][4],nu1=Par[[id]][5],mu2=Par[[id]][6], sigma2= Par[[id]][7],nu2=Par[[id]][8])


}
save(p_ks,p_x2, file="GOF/p_gof_zidel2.rda")
