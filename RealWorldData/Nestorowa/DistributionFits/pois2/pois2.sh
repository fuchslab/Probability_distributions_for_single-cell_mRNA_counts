#!/bin/sh
#$ -e /home/icb/harsha.kumar/pois2_err.out
#$ -o /home/icb/harsha.kumar/pois2.out
#$ -hard
#$ -l job_mem=8G
#$ -t 1-1000
...
outputFile <- "$WORKDIR/pois2_fits.rds"
if(file.exists(outputFile)) {
  pois2_fits <- readRDS(outputFile)
  all_ids <- setdiff(rownames(d), names(pois2_fits))
} else {
  all_ids <- rownames(d)
}
n <- length(all_ids)
...
for(i in 1:block_size) {
  if(1000*(i-1)+$SGE_TASK_ID <= n) {
    ...
    f <- scModels::fit_params(d[id,], "pois2")
    saveRDS(f, file=paste0("$WORKDIR/pois2_fits/",id,".rds"))
  }
}
EOF
