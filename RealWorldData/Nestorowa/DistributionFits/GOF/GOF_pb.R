...
for(id in names(which(comparison_all_name=="pb")) ){
    ...
    p1 <- dpb(0:max(main_data[id,]),alpha=Par[[id]][1],beta=Par[[id]][2], c=Par[[id]][3], log = FALSE)
...
p_ks[[id]] <- ks.test(main_data[id,],"ppb",alpha=Par[[id]][1],beta=Par[[id]][2],c=Par[[id]][3])

save(p_ks,p_x2, file="DistributionFits/GOF/p_gof_pb.rda")
}
