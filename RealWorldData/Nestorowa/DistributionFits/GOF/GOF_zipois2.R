...
#Pois2
pzipois2 <- function(q, p_1, p_2, lambda1, lambda2){
    return(p_1 * (q>=0) + p_2 * ppois(q,lambda1)+(1-p_1-p_2)*ppois(q, lambda2))
}
dzipois2 <- function(x, p_1, p_2, lambda1, lambda2){
    return(p_1 * (x==0)+ p_2 * dpois(x,lambda1)+(1-p_1-p_2)*dpois(x, lambda2))
}
...
for(id in names(which(comparison_all_name=="zipois2")) ){
    ...
    p1 <- dzipois2(0:max(main_data[id,]),p_1=Par[[id]][1],p_2=Par[[id]][2],lambda1=Par[[id]][3], lambda2= Par[[id]][4])
    ...
    p_ks[[id]] <- ks.test(main_data[id,],"pzipois2",p_1=Par[[id]][1],p_2=Par[[id]][2],lambda1=Par[[id]][3], lambda2= Par[[id]][4])


}
save(p_ks,p_x2, file="DistributionFits/GOF/p_gof_zipois2.rda")
