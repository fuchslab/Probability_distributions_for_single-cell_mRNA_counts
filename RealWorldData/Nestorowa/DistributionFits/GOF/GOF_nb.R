...
for(id in names(which(comparison_all_name=="nb")) ){
    ...
    p1 <- dnbinom(0:max(main_data[id,]),size=Par[[id]][1],mu=Par[[id]][2])
...
p_ks[[id]] <- ks.test(main_data[id,],"pnbinom",size=Par[[id]][1],mu=Par[[id]][2])


}

save(p_ks,p_x2, file="DistributionFits/GOF/p_gof_nb.rda")
