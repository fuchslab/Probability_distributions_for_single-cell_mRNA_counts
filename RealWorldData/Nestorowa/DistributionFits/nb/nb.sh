#!/bin/sh
#$ -e /home/icb/harsha.kumar/nb_err.out
#$ -o /home/icb/harsha.kumar/nb.out
#$ -hard
#$ -l job_mem=8G
#$ -t 1-1000
...
outputFile <- "$WORKDIR/nb_fits.rds"
if(file.exists(outputFile)) {
  nb_fits <- readRDS(outputFile)
  all_ids <- setdiff(rownames(d), names(nb_fits))
} else {
  all_ids <- rownames(d)
}
n <- length(all_ids)
...
for(i in 1:block_size) {
  if(1000*(i-1)+$SGE_TASK_ID <= n) {
    ...
    f <- scModels::fit_params(d[id,], "nb")
    saveRDS(f, file=paste0("$WORKDIR/nb_fits/",id,".rds"))
  }
}
EOF
