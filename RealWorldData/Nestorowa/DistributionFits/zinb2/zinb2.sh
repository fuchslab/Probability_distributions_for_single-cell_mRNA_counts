#!/bin/sh
#$ -e /home/icb/harsha.kumar/zinb2_err.out
#$ -o /home/icb/harsha.kumar/zinb2.out
#$ -hard
#$ -l job_mem=8G
#$ -t 1-1000
...
outputFile <- "$WORKDIR/zinb2_fits.rds"
if(file.exists(outputFile)) {
  zinb2_fits <- readRDS(outputFile)
  all_ids <- setdiff(rownames(d), names(zinb2_fits))
} else {
  all_ids <- rownames(d)
}
n <- length(all_ids)
...
for(i in 1:block_size) {
  if(1000*(i-1)+$SGE_TASK_ID <= n) {
    ...
    f <- scModels::fit_params(d[id,], "zinb2")
    saveRDS(f, file=paste0("$WORKDIR/zinb2_fits/",id,".rds"))
  }
}
EOF
