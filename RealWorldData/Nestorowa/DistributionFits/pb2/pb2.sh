#!/bin/sh
#$ -e /home/icb/harsha.kumar/pb2_err.out
#$ -o /home/icb/harsha.kumar/pb2.out
#$ -hard
#$ -l job_mem=8G
#$ -t 1-1000
...
outputFile <- "$WORKDIR/pb2_fits.rds"
if(file.exists(outputFile)) {
  pb2_fits <- readRDS(outputFile)
  all_ids <- setdiff(rownames(d), names(pb2_fits))
} else {
  all_ids <- rownames(d)
}
n <- length(all_ids)
...
for(i in 1:block_size) {
  if(1000*(i-1)+$SGE_TASK_ID <= n) {
    ...
    f <- scModels::fit_params(d[id,], "pb2")
    saveRDS(f, file=paste0("$WORKDIR/pb2_fits/",id,".rds"))
  }
}
EOF
